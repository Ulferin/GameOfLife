 ...
 * ---------- Sequential version ----------
 * 
 * This file represents the sequential implementation of
...
bool updateCell(int r, int c, Board board) {
  ...
  for(auto offrow : range) {
    // Board's boundary check
    if((r+offrow >= 0) && (r+offrow < board.size())) {
      Row row = board.at(r + offrow);
      for(auto offcol : range) {
        if( (offcol || offrow) && (c+offcol >= 0) && (c+offcol < row.size())) {
          neighbours += board.at(r+offrow).at(c+offcol);
        }
      }
    }
  }
...
{
  if(argc != 6) {
    std::cout << "Usage: " << argv[0] << " nrows ncols iters seed sleeptime\n";
    return -1;
  }
